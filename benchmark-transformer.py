...
def generate_autoregressive_slow(
    input_ids: Tensor, model: Transformer, num_new_tokens: int
):
...
def prefill_fx(model: Transformer, fx, input_pos, rope_pos, block_mask):
...
def generate_chunk(model: Transformer, fx, input_pos, rope_pos, block_mask):
    # fx: (B, 1, D)
    # chunk_idx: [1]
