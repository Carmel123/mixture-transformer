...
class FFNTransformer(nn.Module):
    def __init__(self, config: FFNTransformerConfig) -> None: 
        super().__init__()
        self.config = config

        self.wte = nn.Embedding(config.padded_vocab_size, config.dim)
        self.layers = nn.ModuleList(FFNTransformerBlock(config, layer_idx=i) for i in range(config.n_layer))
...
class FFNTransformerBlock(nn.Module):
    def __init__(self, config: FFNTransformerConfig, layer_idx: int) -> None:
        super().__init__()
        self.config = config
        ...
        if config.use_fused_ops:
            self.experts = nn.ModuleList(LigerSwiGLUMLP(config, layer_idx=i) for i in range(config.n_expert))
        else:
            self.experts = nn.ModuleList(LLaMAMLP(config, layer_idx=i) for i in range(config.n_expert))
    ...
    def forward(self, x: Tensor, cos: Tensor, sin: Tensor, is_causal: Optional[bool] = True, mask: Optional[BlockMask] = None, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        # expert outputs
        exp_out = torch.stack(
            [expert(self.ffn_norm(h)) for expert in self.experts], dim=2)
